...
int main() {
	...
	for (;;) {
		if (system.get_echo())
			std::cout << system.folder_to_str(system.get_curr_loc());
		std::cout << ">";
...
void getCommand(System& sys, std::string& _input) {
	...
	for (; _input[f] == ' ' && f < _input.length(); ++f) //Skip the space between command and instructions
		;
	...
	else if (command == "tree" || command == "tr")
		sys.print_folder_tree(*sys.get_curr_loc());
	else if (command == "cd" || command == "changedir" || command == "chdir")
		sys.change_dir(instructions);
	else if (command == "cl" || command == "color")
		sys.color_console(instructions);
	else if (command == "help" || command == "h")
		sys.print_help();
	else if (command == "echo")
		sys.echo(instructions);
	else if (command == "..") {
